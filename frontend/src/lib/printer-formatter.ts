...
export function formatOrderReceipt(printer: any, order: OrderData): void {
  ...
  printer.addText(`注文時刻: ${new Date(order.orderTime).toLocaleString('ja-JP')}\n`)
  printer.addText(`決済状況: ${order.paymentStatus === 'paid' ? '決済済み' : '未決済'}\n\n`)
  ...
  order.items.forEach((item, index) => {
    ...
    printer.addText(`   数量: ${item.quantity} × ¥${item.unitPrice} = ¥${item.totalPrice}\n`)
    if (item.notes) {
...
export function formatManagementReceipt(printer: any, order: OrderData): void {
  ...
  try {
    printer.addTextAlign(printer.ALIGN_CENTER)
    printer.addSymbol(order.orderId, printer.SYMBOL_QRCODE_MODEL_2, 
                     printer.LEVEL_M, 5, 5, 200)
    printer.addText('\n')
  } catch (error) {
